...
export default function BlogClient({ staticData }: BlogProps) {
  ...
  const heroContent = useInView({ threshold: 0.3 });
  const heroImage = useInView({ threshold: 0.3 });
  const featuredHeader = useInView({ threshold: 0.3 });
  const featuredMainCard = useInView({ threshold: 0.2 });
  const featuredSideCards = useInView({ threshold: 0.2 });
  const discoverHeader = useInView({ threshold: 0.3 });
  const newsletterContent = useInView({ threshold: 0.3 });

  return (
    <section className="relative w-full min-h-screen bg-[#CCC1B4]">
      <div className="max-w-full">
        ...
        >
          ...
          <div
            ref={heroContent.ref}
            className={`w-full lg:w-[38.33vw] flex flex-col items-start justify-center gap-[4.44vw] lg:gap-[1.67vw] max-w-full lg:max-w-[52.08vw] lg:order-1 transition-all duration-1000 ease-out delay-200 ${
              heroContent.isInView
                ? "opacity-100 translate-y-0"
                : "opacity-0 translate-y-8"
            }`}
          >
            ...
            <div className="flex flex-col items-center justify-center gap-[1.11vw] lg:gap-[0.14vw] group">
              ...
              <div className="w-full h-[0.56vw] lg:h-[0.28vw] rounded-[0.28vw] lg:rounded-[0.07vw] bg-[#0a0a0a] transition-colors duration-300 group-hover:bg-[#0a5060]" />
            </div>
          </div>
        </section>
        ...
        <section
          className="w-full lg:h-[62.5vw] 2xl:h-[62.5vw] bg-gradient-to-br from-[#13181c] to-[#0a5060] px-[16px] lg:px-[2.78vw] py-[80px] lg:py-[8.33vw]"
          style={{
            background:
              "linear-gradient(305deg, #13181C 35.5%, #28A0AA 75.23%, #BEEBEE 120.85%)",
          }}
        >
          <div className="mx-auto">
            {/* Header */}
            <div
              ref={featuredHeader.ref}
              className={`flex flex-col lg:flex-row lg:items-start lg:justify-between gap-[24px] lg:gap-[1.67vw] mb-[16px] lg:mb-[5.83vw] transition-all duration-1000 ease-out ${
                featuredHeader.isInView
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-8"
              }`}
            >
            ...
            <div className="flex flex-col lg:flex-row gap-[16px] lg:gap-[1.11vw]">
              {/* Main Featured Card */}
              <div
                ref={featuredMainCard.ref}
                className={`rounded-[4.44vw] lg:rounded-[1.67vw] overflow-hidden flex flex-col lg:flex-row bg-[#f2e9da] transition-all duration-1000 ease-out delay-200 ${
                  featuredMainCard.isInView
                    ? "opacity-100 translate-y-0"
                    : "opacity-0 translate-y-8"
                }`}
              >
              ...
              {/* Side Blog Cards */}
              <div
                ref={featuredSideCards.ref}
                className={`flex flex-col lg:flex-row gap-[4.44vw] lg:gap-[1.11vw] transition-all duration-1000 ease-out delay-400 ${
                  featuredSideCards.isInView
                    ? "opacity-100 translate-y-0"
                    : "opacity-0 translate-y-8"
                }`}
              >
        ...
        <section className="w-full bg-[#fbfbfb] px-0 pt-[80px]">
          <div className="w-full flex flex-col items-start gap-[40px]">
            {/* Header Section */}
            <div
              ref={discoverHeader.ref}
              className={`w-full flex flex-col items-center gap-[24px] transition-all duration-1000 ease-out ${
                discoverHeader.isInView
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-8"
              }`}
            >
        ...
        <section className="relative w-full h-full py-20 md:py-16 lg:py-20 2xl:py-[6.94vw]">
          ...
          {/* Content */}
          <div
            ref={newsletterContent.ref}
            className={`relative z-10 w-full mx-auto px-6 sm:px-8 lg:px-[120px] 2xl:px-[8.33vw] flex flex-col lg:flex-row items-center gap-16 lg:gap-24 2xl:gap-[8.33vw] transition-all duration-1000 ease-out ${
              newsletterContent.isInView
                ? "opacity-100 translate-y-0"
                : "opacity-0 translate-y-8"
            }`}
          >
